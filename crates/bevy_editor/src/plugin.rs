...
use bevy::camera::{OrthographicProjection, PerspectiveProjection, Projection, ScalingMode, Viewport};
...
#[derive(Resource, Default)]
struct AxisAlignedProjectionState {
...
fn matched_orthographic_from_perspective(perspective: &PerspectiveProjection) -> OrthographicProjection {
...
fn sync_axis_aligned_projection(
    ...
    if state.active_camera != Some(camera_entity) {
        ...
        let orthographic = match state.saved_projection.as_ref().unwrap_or(&*projection) {
            Projection::Perspective(perspective) => matched_orthographic_from_perspective(perspective),
        ...
        *projection = Projection::Orthographic(orthographic);
        ...
        if let Some(Projection::Perspective(perspective)) = state.saved_projection.as_ref() {
            *projection = Projection::Orthographic(matched_orthographic_from_perspective(perspective));
        }
    }
}
